  ...
  function initEmbeds() {
    ...
    document.querySelectorAll("p").forEach(p => {
      ...
      while ((match = regex.exec(p.innerHTML)) !== null) {
        ...
        const cardImgUrl = `https://images.pokemontcg.io/${set}/${number}.png`;
        const hiResUrl = `https://images.pokemontcg.io/${set}/${number}_hires.png`;

        container.innerHTML = `
          <div class="poke-card-image">
            <img src="${cardImgUrl}" alt="${name}" data-hires="${hiResUrl}" />
          </div>
          <div class="poke-info">
            <h3>${name}</h3>
            <div class="poke-rarity" data-rarity></div>
        ...
        loadRarity(id, container);
      }
    });
  }

  async function loadRarity(cardId, container) {
    try {
      const res = await fetch(`${POKEMON_API}${cardId}`);
      const data = await res.json();
      const rarity = data?.data?.rarity;
      if (rarity) {
        const rarityDiv = container.querySelector("[data-rarity]");
        if (rarityDiv) rarityDiv.textContent = `(${rarity})`;
      }
    } catch (err) {
      console.warn("Could not fetch rarity for", cardId);
    }
  }
